...
C     open(7006,FIlE='parastos2.s12s2')
 ...
 3555 CONTINUE
   55 CONTINUE                            ! 设计方案跳转
      ...
      ! 将亚音速级初始叶片厚度分布传入X,Y两个数组
      IF(KF.EQ.1) GOTO 54                 ! KF：亚音速级初始叶片形状类型符号
      IF(KF.EQ.2) GOTO 53
      DO 52 I=1,16                        ! BC-10叶形
      X(I)=X3(I)
      Y(I)=Y3(I)
   52 CONTINUE
      GOTO 54
   53 DO 51 I=1,16                        ! C-4叶形
      X(I)=X2(I)
      Y(I)=Y2(I)
   51 CONTINUE
   54 CONTINUE
...
C      open(7010,FILE='bladevars.s12s1')
  ...
  881 B(2,L)=B(2,L)*1000.     ! 导向器出口外缘直径
 ...
 6475   CONTINUE
  ...
  410 CONTINUE
      ...
      STAGE(22)=PPI
      FX(22)=PPI
  10  DO 1 I=IIS,IIF      ! 级循环开始，II=IIS
      INDEXC=0
      MG=IIS/I
      IDQC=(1-MG)*IDQC
      CALL PE4ATI(IPE4,KPAH,FKPAH,KRAN,I,AN,M,PP,AKPX,IG)     ! 检查参数范围是否符合要求，并生成输出控制数组
      UKZ=UKP
      IF(AN(M,3).EQ.0.) GO TO 811     ! 默认为0，AN(M,3)作用未知
      TKT=1
      UKZ=UKM*MG+UKP*(1-MG)           ! ?????
      IF(I.NE.IKBD) GO TO 811         ! IKBD=0
  ...
  203 CONTINUE
   ...
   11 DO 2 K=1,20
    ...
    2 HA(K)=B(K,I)
      ...
      RK(16)=0.
  800 CONTINUE
      IF(INDEX4.LE.0)GOTO 35      ! 跳至计算能头、效率等
      IF(I.GT.IIS)GO TO 35        ! 第一级需要计算至35
   37 DO 38 L=1,24
      STAGE(L)=FX(L)
   38 C(1+L/7)=ZC(1+L/7)
      DSITAK=W1
 ...
 2101 IZY=8*IIS-7
      ...
      IF(AKPDMA.GT.AKPX) GOTO 1413
        ...
        STORE(329)=AKPX
        LJ=0
        DO 1412 LI=330,489        ! STORE的第330至489号用于存放STUP数组
          LJ=LJ+1
 ...
 2102 PW=PP*MW+PW*(1-MW)
  ...
  220 GB(1)=PP1
      ...
      GB(4)=ARFA1
      UKP=UKM
      IF(IQP.GT.0.AND.IQP.NE.3) GOTO 820      ! IQP=0, STEPIN->STEPAT>STEPON
     ...
     *DSISET,SDELTQ,WCP,EP,GUIDE,SGUIDE,KSTEP)
      ...
      IF(INDEXZ.NE.2) GOTO 842                ! INDEXZ=0
      MG=INDEXZ/2
      INDEXZ=1+2*MG                           ! INDEXZ=1
  833 CONTINUE
      STORE(1)=GPP
      STORE(2)=PP
      STORE(3)=AKPX
  ...
  842 MG=(QGP+4)/(AN(M,2)+4)*INDEXB/4
      ...
      GO TO (820,56,10,666),KSTEP             ! 820：STEPAT；56：重新开始级循环；10：重新开始级循环；666：特性线计算结束
 ...
 852  STORE(164)=GPP
  ...
  805 CALL STEPON(M,IQP,IQ,IDQC,INDEXQ,INDEX2,INDEX4,INDEX6,INDEX7,
     ...
     *KM,INDEX8,IND8R,INDEXB,KSTEP,BRANCH,POINTS)
      ...
      IGG=IGG*MG
      LAR=6
      GO TO (9,232,666,66,57),KSTEP           ! 9：下一步；232：跳过IQP判断；666,66：特性线计算结束；57：56之前
  ...
  605 FORMAT('   EXIT   结束')
   ...
   12   R=F(1)
     ...
     *C2AOA,ALFA4,LA4,C4A,II)  !静叶出口计算
      ...
      WW5=1.-BIS(II)
      QLA4=QLA1*WW1*WW2*WW3*WW4*WW5/PI    ! 出口折合流量计算
      LA4=RLQMDA(QLA4)
